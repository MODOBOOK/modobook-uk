-- Deep-link medical-form notifications to the specific patient page,
-- and make the allergy sync overwrite existing values from the latest form.
...
BEGIN
  IF NEW.status = 'submitted' AND COALESCE(OLD.status::text,'') <> 'submitted' THEN
    -- Resolve the clinic_clients row the form belongs to (mirrors sync trigger)
    v_client_id := NEW.client_id;
    IF v_client_id IS NULL AND NEW.appointment_id IS NOT NULL THEN
    ...
    v_link := CASE WHEN v_client_id IS NOT NULL
                   THEN '/dashboard/patients/' || v_client_id::text
                   ELSE '/dashboard/patients' END;

    PERFORM public.create_notification(
      NEW.profile_id, 'form',
      'Medical form completed',
      'A patient just submitted a medical form.',
      '📝', v_link, NEW.id, 'medical_form'
    );
  END IF;
  RETURN NEW;
END; $function$;

-- Refresh allergy + core medical fields from the latest submitted form.
...
BEGIN
  ...
  UPDATE public.clinic_clients
     SET medical_form_data = v_resp,
         ...
         -- Prefer the latest form's allergy answer when the patient provided one.
