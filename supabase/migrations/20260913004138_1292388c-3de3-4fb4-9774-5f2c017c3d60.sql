...
BEGIN
  BEGIN
    v_fix := public.apply_health_auto_fixes();
  ...
  IF v_fix IS NOT NULL AND coalesce((v_fix->>'skipped')::boolean, true) = false THEN
    UPDATE public.health_findings f
       SET auto_fixed_count = coalesce(f.auto_fixed_count,0) + (x->>'fixed')::int,
           last_auto_fixed_at = now(),
           updated_at = now()
      FROM jsonb_array_elements(v_fix->'results') x
     WHERE f.check_key = x->>'check_key'
       AND coalesce((x->>'fixed')::int, 0) > 0;
  END IF;
