...
CREATE OR REPLACE FUNCTION public.save_walk_in_medical_form_response(
  p_referral_id uuid,
  p_form_id uuid,
  p_response jsonb
) RETURNS boolean
...
BEGIN
  ...
  IF v_ref.status NOT IN ('accepted', 'completed') THEN
    RAISE EXCEPTION 'Accept the case before completing medical forms';
  END IF;
  ...
  UPDATE public.appointment_medical_forms amf
  SET response = p_response,
      status = 'submitted',
      submitted_at = COALESCE(submitted_at, now())
  WHERE amf.id = p_form_id
    AND amf.profile_id = v_ref.practitioner_profile_id
    AND (
      (v_ref.appointment_id IS NOT NULL AND amf.appointment_id = v_ref.appointment_id)
      OR (v_client IS NOT NULL AND amf.client_id = v_client)
      OR (v_ref.patient_email IS NOT NULL AND lower(COALESCE(amf.recipient_email, '')) = lower(v_ref.patient_email))
    );

  IF NOT FOUND THEN
    RAISE EXCEPTION 'Medical form is not attached to this walk-in';
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.save_walk_in_medical_form_response(uuid, uuid, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.save_walk_in_medical_form_response(uuid, uuid, jsonb) TO authenticated;