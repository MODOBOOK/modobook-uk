DROP FUNCTION IF EXISTS public.list_clinic_visits_for_slug(text, uuid[]);
CREATE OR REPLACE FUNCTION public.list_clinic_visits_for_slug(p_slug text, p_treatment_ids uuid[])
 RETURNS TABLE(visit_id uuid, treatment_id uuid, prescriber_user_id uuid, prescriber_name text, location_id uuid, location_name text, visit_date date, start_time time without time zone, end_time time without time zone, remaining_capacity integer, total_capacity integer, notes text)
 LANGUAGE sql
...
$function$;
GRANT EXECUTE ON FUNCTION public.list_clinic_visits_for_slug(text, uuid[]) TO anon, authenticated, service_role;