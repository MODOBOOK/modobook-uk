...
BEGIN
  ...
  IF v_ref.client_id IS DISTINCT FROM v_client THEN
  ...
  WHERE NOT EXISTS (
    ...
    WHERE existing.profile_id = v_ref.practitioner_profile_id
      ...
      AND (
        (v_ref.appointment_id IS NOT NULL AND existing.appointment_id = v_ref.appointment_id)
        OR (
          v_ref.appointment_id IS NULL
