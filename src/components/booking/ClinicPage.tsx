...
function WelcomeIntroBlock({
  ...
  const proseCls =
    (variant === "mobile" ? "text-sm leading-relaxed" : "text-base leading-relaxed") +
    " [&_p]:mb-3 [&_p:last-child]:mb-0 [&_p:empty]:min-h-[1em] [&_p:has(>br:only-child)]:min-h-[1em] [&_ul]:mb-3 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:mb-3 [&_ol]:list-decimal [&_ol]:pl-5";
  return (
    <>
      {heading && (
        <h2
...
export function ClinicPage({ data, view, slug }: { data: ClinicPageData; view: ClinicPageView; slug: string }) {
  ...
  // Contact ("Get in touch") and booking-policy sections. On pages with the
  // quick-book CTA these render below the treatment menu instead of up top.
  const contactSectionNode = (() => {
