...
export const upsertClientNote = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { id?: string; client_id: string; body: string }) => d)
  .handler(async ({ data, context }) => {
    ...
    if (data.id) {
      const { error } = await context.supabase.from("client_notes")
        .update({ body: data.body }).eq("id", data.id);
      if (error) throw error;
      return { ok: true };
    }
    const { error } = await context.supabase.from("client_notes")
      .insert({ profile_id: pid, client_id: data.client_id, body: data.body });
    if (error) throw error;
    return { ok: true };
  });
