...
export const listStaff = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await context.supabase
      .from("staff_members")
      .select("id, name, invited_email, role, data_scope, practitioner_id, status, can_manage_rota, can_use_prescribing, payout_mode, commission_percent, deduct_product_cost, stripe_account_id, stripe_account_status, invited_at, accepted_at, last_active_at, invite_expires_at")
