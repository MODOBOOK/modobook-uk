...
export const getDashboardAnalytics = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: appointments, error: apptErr } = await supabase
      .from("appointments")
      .select(
        "id, patient_name, patient_email, patient_user_id, scheduled_date, start_time, end_time, status, payment_status, total_amount, amount_paid_cents, amount_refunded_cents, checkout_discount_cents, treatments(name, color, category_id), treatment_categories(name)",
      )
