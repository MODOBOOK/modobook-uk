...
export const listMyAppointments = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    let q = supabase
      .from("appointments")
      .select("id, patient_name, patient_email, patient_phone, scheduled_date, start_time, end_time, status, payment_status, total_amount, amount_paid_cents, amount_refunded_cents, checkout_discount_cents, checkout_notes, deposit_paid_at, stripe_payment_intent_id, card_capture_agreed_at, card_captured_at, card_capture_policy_text, checked_out_at, notes, practitioner_notes, aftercare_html, has_allergies, allergies_text, treatment_id, treatment_name_snapshot, location_id, payment_hold_expires_at, practitioner_id, treatments(name, color), locations(name), practitioners(name)")
      .eq("profile_id", profileId)
