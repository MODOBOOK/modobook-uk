...
export const Route = createFileRoute("/_authenticated/hub/referrals")({
  ssr: false,
  component: HubReferrals,
});

type Ref = Awaited<ReturnType<typeof listMyReferrals>>[number];

function HubReferrals() {
  ...
  async function act(id: string, action: "accept" | "decline" | "complete") {
    try {
      ...
      toast.success(
        action === "accept" ? "Case accepted" : action === "decline" ? "Declined" : "Marked complete",
      );
      refs.refetch();
    } catch (e) {
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Patients book treatments that need prescriber sign-off; their case appears here. Accept to
          unlock the full medical record.
        </p>
      </div>
          ...
          <>
            ...
            >
              <span className="inline-flex items-center gap-2">
                <User className="h-4 w-4" /> Full record &amp; medical forms
              </span>
...
function FullRecord({ id, onComplete, status }: { id: string; onComplete: () => void; status: Ref["status"] }) {
  ...
  const q = useQuery({
    queryKey: ["referral-full", id],
    queryFn: () => fetchFull({ data: { id } }),
  });
  ...
  const forms: { id: string; template_name: string; response: unknown; submitted_at: string | null; status: string }[] =
    parsed.medical_forms ?? [];
  const consultations: Record<string, unknown>[] = parsed.consultations ?? [];
    ...
    <div className="space-y-4 rounded-md border bg-muted/30 p-3 text-sm">
      ...
      {appt && (
        <section>
          ...
          <p className="mt-1">
            {appt.scheduled_date} at {String(appt.start_time).slice(0, 5)} · {appt.status}
          </p>
        </section>
      )}
      ...
      <section>
        ...
        {forms.length === 0 ? (
          <p className="mt-1 text-muted-foreground">None submitted yet.</p>
        ) : (
        ...
        <section>
          ...
          <ul className="mt-1 space-y-2">
            {consultations.map((c, i) => (
              <li key={String((c as { id?: string }).id ?? i)} className="rounded border bg-background p-2 text-xs">
                <pre className="overflow-x-auto whitespace-pre-wrap break-words">{JSON.stringify(c, null, 2)}</pre>
              </li>
            ))}
          </ul>
        </section>
      ...
      {status === "accepted" && (
        <section className="space-y-2">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            Prescribing note (internal)
          </p>
          <Textarea
            rows={2}
            value={note}
            onChange={(e) => setNote(e.target.value)}
            placeholder="Decision, dose, instructions…"
          />
